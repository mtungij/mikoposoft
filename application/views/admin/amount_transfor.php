    ...
    <div class="grid grid-cols-3 gap-4">
        <?php foreach ($account as $accounts): ?>
            <div class="card">
                <div class="card-body depostit-card p-5">
                    <div class="depostit-card-media flex justify-between ">
                        <div>
                            <h6 class="font-normal uppercase"><?= $accounts->account_name?></h6>
                            <h3 class="font-semibold leading-[1.346]"><?= number_format($accounts->total_amount) ?></h3>
                        </div>
                    ...
                    <div class="progress-box mt-0">
                        <div class="flex justify-between">
                            <p class="mb-0">Main Account Balance</p>
        ...
        <?php endforeach; ?>
						...
						<div class="card flex flex-col max-sm:mb-[30px] profile-card">
                            <h4 class="ml-4 px-4 pt-2">Add Branch Float</h4>
                            <hr>
								...
								<div class="sm:p-10 sm:pb-2.5 p-[25px] pb-0">
									<div class="row">
										<div class="sm:w-1/2 w-full mb-[30px]">
											<label class="text-dark dark:text-white text-[13px] mb-2"><small class="text-danger">*</small>From Main Account</label>
											<select type="number" name="from_trans_id" class="relative text-[13px] h-[2.813rem] border border-b-color block rounded-md py-1.5 px-3 duration-500  outline-none w-full">
                                        <option value="">Select Account</option>
                                        <?php foreach ($accountNames as $accounts): ?>
                                        <option value="<?php echo $accounts->trans_id; ?>"><?= $accounts->account_name ; ?></option>
										
                                        <?php endforeach; ?>
                                    </select>
										</div>
										<div class="sm:w-1/2 w-full mb-[30px]">
											<label class="text-dark dark:text-white text-[13px] mb-2"><small class="text-danger">*</small>Maximum Loan Amount</label>
                                            <input x-data x-mask:dynamic="$money($input)" name="blanch_amount"  class="form-control relative text-[13px] text-body-color h-[2.813rem] border border-b-color block rounded-md py-1.5 px-3 duration-500  outline-none w-full"required>
										</div>
										<div class="sm:w-1/2 w-full mb-[30px]">
                                        <label class="text-dark dark:text-white text-[13px] mb-2"><small class="text-danger">*</small>From Main Account</label>
									 <select type="number" name="branch_id" class="relative text-[13px] h-[2.813rem] border border-b-color block rounded-md py-1.5 px-3 duration-500  outline-none w-full">
										...
										<div class="sm:w-1/2 w-full mb-[30px]">
                                        <label class="text-dark dark:text-white text-[13px] mb-2"><small class="text-danger">*</small>To Branch Account</label>
									 <select type="number" name="to_trans_id" class="relative text-[13px] h-[2.813rem] border border-b-color block rounded-md py-1.5 px-3 duration-500  outline-none w-full">
                                        <option value="">select branch Account</option>
                                        <?php foreach ($accountNames as $accounta): ?>
										<option value="<?php echo $accounta->trans_id; ?>"><?php echo $accounta->account_name; ?></option>
										<?php endforeach; ?>
                                    </select>
										</div>
                                        <div class="sm:w-1/2 w-full mb-[30px]">
											<label class="text-dark dark:text-white text-[13px] mb-2"><small class="text-danger">*</small>Withdraw Chargers</label>
                                            <input x-data x-mask:dynamic="$money($input)" name="charger"  class="form-control relative text-[13px] text-body-color h-[2.813rem] border border-b-color block rounded-md py-1.5 px-3 duration-500  outline-none w-full"required>
										</div>
                                        <?php $date = date("Y-m-d"); ?>
							...
							<div class="card-body p-0">
								<div class="overflow-x-auto active-projects">
									<div class="sm:p-5 p-4">
                                        <h4 class="text-base">Loan Product List</h4>
									</div>
									<table id="example" class="table w-full ">
										<thead>
											<tr>
												...
												<th class="bg-primary-light text-[13px] py-2.5 px-4 text-primary capitalize border-b border-b-color font-medium bg-none whitespace-nowrap text-left">Loan Product Name</th>
												<th class="bg-primary-light text-[13px] py-2.5 px-4 text-primary capitalize border-b border-b-color font-medium bg-none whitespace-nowrap text-left">Minimum Loan Amount</th>
												<th class="bg-primary-light text-[13px] py-2.5 px-4 text-primary capitalize border-b border-b-color font-medium bg-none whitespace-nowrap text-left">Maximum Loan Amount</th>
												<th class="bg-primary-light text-[13px] py-2.5 px-4 text-primary capitalize border-b border-b-color font-medium bg-none whitespace-nowrap text-left">Loan Product Interest</th>
                                        ...
                                        <?php foreach ($loan_category as $loan_categorys): ?>
												...
												<td class="text-[13px] font-normal uppercase text-body-color py-2.5 pl-4 pr-0 whitespace-nowrap border-b border-[#E6E6E6] dark:border-[#ffffff1a]"><?= $loan_categorys->loan_name ?></td>
												<td class="border-b border-[#E6E6E6] dark:border-[#ffffff1a] text-[13px] text-body-color py-2.5 px-5 font-normal whitespace-nowrap"><?= number_format($loan_categorys->loan_price)?></td>
												<td class="border-b border-[#E6E6E6] dark:border-[#ffffff1a] text-[13px] text-body-color py-2.5 px-5 font-normal whitespace-nowrap"><?= number_format($loan_categorys->loan_perday) ?></td>
												<td class="border-b border-[#E6E6E6] dark:border-[#ffffff1a] text-[13px] text-body-color py-2.5 px-5 font-normal whitespace-nowrap"><?= $loan_categorys->interest_formular  ?>%</td>
                                            ...
                                            <td class="border-b border-[#E6E6E6] dark:border-[#ffffff1a] text-right py-2.5 px-5 whitespace-nowrap">
												<div class="md:w-1/4 sm:w-1/3 w-1/2">
                                                    <div class="dropdown custom-dropdown inline-block mb-4">
                                                        <div class="btn min-w-[2.4rem] p-[0.4375rem] h-[2.4rem] leading-[1.7] min-h-[2.5rem] btn-primary tp-btn ml-4 hover:bg-primary rounded-md dz-dropdown" data-dz-dropdown="<?php echo $loan_categorys->category_id ?>">
                                                        ...
                                                        <div class="dz-dropdown-menu absolute translate-x-[-167px] translate-y-2.5 overflow-hidden rounded-md z-[9] bg-white dark:bg-[#182237] shadow-[0_0_3.125rem_0_rgba(82,63,105,0.15)] min-w-[13rem] text-left py-2 hidden" id="<?php echo $loan_categorys->category_id ?>">
