	...
	public function aprove_disbas_statusNotloanfee($loan_id){
       ...
       if($loan_data->rate == 'FLAT RATE'){
	     ...
	     $months = floor($day_data / 30);
      	 ...
      	 $interest = $interest_loan;
      	 $loan_interest = $interest /100 * $loan_aproved * $day_data;
      	 $total_loan = $loan_aproved + $loan_interest; 
      	 $restoration = ($loan_interest + $loan_aproved) / ($session_loan);
      	 $res = $restoration;
	  ...
	  $phones = $loan_datas->phone_no;
           ...
           //send sms function
         $sms = $comp_name.' Imeingiza Mkopo Kiasi cha Tsh.'.$remain_makato.' kwenye Acc Yako ' . $loan_codeID .' Kwa msaada zaidi Piga simu Namba '.$comp_phone;
         $massage = $sms;
         $phone = $phones;
               // print_r($massage);
               //     exit();
	...
	$this->form_validation->set_error_delimiters('<div class="text-danger">','</div>');
       ...
       //     echo "<pre>";
        ...
        $this->depost_balance($loan_id,$comp_id,$blanch_id,$customer_id,$new_depost,$sum_balance,$description,$role,$group_id,$p_method,$deposit_date,$dep_id);
	     ...
	     $remain_loan = $loan_int - $total_depost->remain_balance_loan;
	        //sms send
          $sms = 'Umeingiza Tsh.' .number_format($new_balance). ' kwenye Acc Yako ' . $loan_codeID .''. $comp_name.' Mpokeaji '.$role . ' Kiasi kilicho baki Kulipwa ni Tsh.'.number_format($remain_loan).' Kwa malalamiko piga '.$comp_phone;
          $massage = $sms;
          $phone = $phones;
...
public function previous_transfor(){
 ...
 public function cash_transaction_blanch(){
  ...
  //   echo "<pre>";
    ...
    $empl_data = $this->queries->get_employee_data($empl_id);
    $mpdf = new \Mpdf\Mpdf();
    $html = $this->load->view('admin/previous_cash_report',['compdata'=>$compdata,'data'=>$data,'total_cashDepost'=>$total_cashDepost,'total_withdrawal'=>$total_withdrawal,'from'=>$from,'to'=>$to,'blanch_data'=>$blanch_data,'empl_data'=>$empl_data,'empl_id'=>$empl_id],true);
    $mpdf->SetFooter('Generated By Brainsoft Technology');
    $mpdf->WriteHTML($html);
    $mpdf->Output();
 ...
 public function update_loan_statatus_adjust_withdrawal($loan_id){
  ...
  $query = $this->db->query($sqldata);
   return true;	
		...
		// public function test_iconload(){

		// 	$this->load->view('admin/test_icon');
		// }
